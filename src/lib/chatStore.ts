...
class ChatStore {
  ...
  getLastMessageInfo(chatId: string): { status?: string; type?: string; isMine?: boolean } | null {
    const messages = this.messages.get(chatId);
    ...
    return {
      ...
      isMine: last.sender_id === this.currentUserId,
    };
  }

  private async preloadLastMessages(chats: Chat[]): Promise<void> {
    if (chats.length === 0) return;
    const chatIds = chats.map(c => c.id);
    try {
      const { data, error } = await supabase
        .from('messages')
        .select('id, chat_id, sender_id, receiver_id, content, message_type, status, created_at')
        .in('chat_id', chatIds)
      ...
      for (const msg of data) {
        if (seen.has(msg.chat_id)) continue;
        seen.add(msg.chat_id);
        const existing = this.messages.get(msg.chat_id) || [];
        if (existing.length === 0) {
          this.messages.set(msg.chat_id, [{
            id: msg.id,
            chat_id: msg.chat_id,
